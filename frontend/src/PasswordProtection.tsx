...
export function PasswordProtection({ children }: { children: React.ReactNode }) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="fixed inset-0 bg-blue text-white flex items-center justify-center z-[999999999]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <input
                ...
                className="w-full px-4 py-3 bg-white/10 border-2 border-white rounded text-white placeholder-white/50 font-mono focus:outline-none focus:border-white/80"
                autoFocus
              />
              {error && (
